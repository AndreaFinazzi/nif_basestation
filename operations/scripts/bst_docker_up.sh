...
docker exec --privileged -w "/workspace" -it nifbasestation_sim \
    /bin/bash -i -c 'colcon build --packages-select nif_msgs deep_orange_msgs bvs_msgs raptor_dbw_msgs joystick_vehicle_interface joystick_vehicle_interface_nodes userinput py_pubsub'
